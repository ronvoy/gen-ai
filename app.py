...
Passenger (via the `passenger_wsgi.py` shim, or directly as cPanel's
"Application startup file"); it also runs with `python app.py`.
"""
...
# WSGI entry point. Plain Passenger reaches it through passenger_wsgi.py;
# cPanel's "Setup Python App" loads this file directly (startup file app.py,
# entry point application). Do NOT name this file passenger_wsgi.py on
# cPanel: the stub it writes under that name would then load itself.
application = app


if __name__ == "__main__":
    app.run(host="127.0.0.1", port=int(os.environ.get("PORT", 8008)), debug=True)
